...
rt_err_t  dev_init(rt_device_t dev) {
    return RT_EOK;
}

rt_err_t  dev_close(rt_device_t dev) {
    return RT_EOK;
}
...
rt_err_t dev_rx_indicate(rt_device_t dev, rt_size_t size) {
...
static rt_size_t  dev_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size) {
    float adcValue = adc_read_voltage();
