# 逻辑
>逻辑是研究推理规则的学问, [对偶原理和蕴含定理](#对偶原理与蕴含定理) 是 **推理代数化** 的两个关键定理。
...
## 蕴含定理的一个对偶语义解释
对于$1 \land \bigwedge p_i \to \bigvee q_j \lor 0 (\text{男}\text{左}\text{女}\text{右})$ 
将  $p$解释为 `p是男人`
把  $q$解释为 `q是男人`
把  $1$解释为 `男雕塑`
把  $0$解释为 `女雕塑`
...
$$
\forall \varepsilon > 0 \, \exists N \, \forall n > N \, (|a_n - A| < s(\varepsilon)) ,\text{其中}\lim_{\varepsilon \to 0} s(\varepsilon) = 0
$$
